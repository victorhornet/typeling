...
alias_decl -> ParseResult<AliasDecl>
    : "ALIAS" "IDENT" "ASSIGN" type "NEWLINE" { Ok(AliasDecl {name: $2?.span(), original: $4?, span: $span}) }
    ;
...
stmt -> ParseResult<Statement>
    : expr "NEWLINE" { Ok(Statement::Expr($1?)) }
...
return_stmt -> ParseResult<Return>
    : "RETURN" expr "NEWLINE" { Ok(Return {value: Some($2?), span: $span}) }
    | "RETURN" "NEWLINE" { Ok(Return {value: None, span: $span}) }
    ;

print_stmt -> ParseResult<Print>
    : "PRINT" "LPAREN" expr "RPAREN" "NEWLINE" { Ok(Print {value: $3?, span: $span}) }
    ;

var_decl -> ParseResult<VarDecl>
    : "IDENT" "COLON" type "NEWLINE" { Ok(VarDecl {name: $1?.span(), var_type: $3?, value: None, span: $span}) }
    | "IDENT" "COLON" type "ASSIGN" expr "NEWLINE" { Ok(VarDecl {name: $1?.span(), var_type: $3?, value: Some($5?), span: $span}) }
    ;

assign_stmt -> ParseResult<Assign>
    : "IDENT" "ASSIGN" expr "NEWLINE" { Ok(Assign {name: $1?.span(), value: $3?, span: $span}) }
    ;
